...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    if (!/^[0-9a-f]{7,64}$/i.test(base)) return json({ error: 'Ugyldig base', code: 'badBase' }, 400);
    ...
    return json({ error: 'Kunne ikke lese repo-status fra GitHub', code: 'statusFailed' }, 502);
  }
}
